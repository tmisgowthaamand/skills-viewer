---
name: pharmacy
description: "Use this skill for any task related to running, starting, or growing a Pharmacy business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: P
---

# 💉 Pharmacy

> Use this skill for any task related to running, starting, or growing a **Pharmacy** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Licensed pharmacy dispensing medicines.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Healthcare Retail |
| **Revenue Model** | Medicine and supplement sales |
| **Key Costs** | Drug inventory, rent, staff |
| **Typical Team Size** | 2–8 |
| **Licenses Needed** | CDSCO drug licence, GST |
| **Primary Tools** | Marg Pharma ERP, PharmEasy B2B, POS |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up Marg Pharma ERP as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
