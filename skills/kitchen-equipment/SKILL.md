---
name: kitchen-equipment
description: "Use this skill for any task related to running, starting, or growing a Kitchen Equipment Store business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: K
---

# 🍳 Kitchen Equipment Store

> Use this skill for any task related to running, starting, or growing a **Kitchen Equipment Store** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Commercial kitchen equipment supply and maintenance.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | B2B Retail/Services |
| **Revenue Model** | Equipment sales, AMC contracts |
| **Key Costs** | Inventory, warehouse, service staff |
| **Typical Team Size** | 3–15 |
| **Licenses Needed** | GST, Shop & Est. |
| **Primary Tools** | IndiaMART, Tally, WhatsApp |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up IndiaMART as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
