---
name: supermarket
description: "Use this skill for any task related to running, starting, or growing a Supermarket business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: S
---

# 🏪 Supermarket

> Use this skill for any task related to running, starting, or growing a **Supermarket** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Large-format grocery and daily needs retail.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Retail |
| **Revenue Model** | Daily FMCG and fresh produce sales |
| **Key Costs** | Inventory, rent, staff, refrigeration |
| **Typical Team Size** | 15–80 |
| **Licenses Needed** | FSSAI, GST, Shop & Est. |
| **Primary Tools** | SAP Retail, POS, inventory management system |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up SAP Retail as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
