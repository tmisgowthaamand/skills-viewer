---
name: beauty-salon
description: "Use this skill for any task related to running, starting, or growing a Beauty Salon & Barber Shop business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: B
---

# 💇 Beauty Salon & Barber Shop

> Use this skill for any task related to running, starting, or growing a **Beauty Salon & Barber Shop** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Full-service beauty and grooming salon.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Personal Services |
| **Revenue Model** | Service fees, product retail |
| **Key Costs** | Rent, staff, products |
| **Typical Team Size** | 3–15 |
| **Licenses Needed** | Shop & Est., GST |
| **Primary Tools** | Fresha, Vagaro, WhatsApp Business |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up Fresha as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
