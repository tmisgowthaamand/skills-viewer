---
name: organic-food
description: "Use this skill for any task related to running, starting, or growing a Organic Food Business business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: O
---

# 🥦 Organic Food Business

> Use this skill for any task related to running, starting, or growing a **Organic Food Business** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Organic food brand and distribution.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Food/FMCG |
| **Revenue Model** | D2C sales, retail distribution |
| **Key Costs** | Production, packaging, certification |
| **Typical Team Size** | 3–20 |
| **Licenses Needed** | NPOP/USDA organic cert, FSSAI |
| **Primary Tools** | Shopify, APEDA portal, WhatsApp, Instagram |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up Shopify as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
