---
name: used-car-dealership
description: "Use this skill for any task related to running, starting, or growing a Used Car Dealership business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: U
---

# 🚙 Used Car Dealership

> Use this skill for any task related to running, starting, or growing a **Used Car Dealership** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Pre-owned vehicle buying and selling.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Automotive Retail |
| **Revenue Model** | Vehicle sales margin, finance income |
| **Key Costs** | Vehicle inventory, reconditioning, rent |
| **Typical Team Size** | 5–20 |
| **Licenses Needed** | RTO transfer authority, GST |
| **Primary Tools** | Cars24 dealer portal, OLX, Excel, Tally |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up Cars24 dealer portal as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
