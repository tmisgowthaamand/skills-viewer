---
name: insurance-agency
description: "Use this skill for any task related to running, starting, or growing a Insurance Agency business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: I
---

# 🛡️ Insurance Agency

> Use this skill for any task related to running, starting, or growing a **Insurance Agency** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Insurance product sales and advisory.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Financial Services |
| **Revenue Model** | Commissions on policies sold |
| **Key Costs** | Staff, compliance |
| **Typical Team Size** | 2–15 |
| **Licenses Needed** | IRDA agent/broker licence |
| **Primary Tools** | IRDA portal, WhatsApp, Excel |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up IRDA portal as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
