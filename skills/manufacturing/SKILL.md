---
name: manufacturing
description: "Use this skill for any task related to running, starting, or growing a Manufacturing business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: M
---

# 🏭 Manufacturing

> Use this skill for any task related to running, starting, or growing a **Manufacturing** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Product manufacturing and distribution.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Manufacturing |
| **Revenue Model** | B2B product sales |
| **Key Costs** | Raw materials, machines, labour |
| **Typical Team Size** | 10–200 |
| **Licenses Needed** | Factory licence, Pollution NOC, GST, BIS |
| **Primary Tools** | SAP/ERP, Tally, Google Sheets |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up SAP/ERP as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
