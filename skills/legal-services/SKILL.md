---
name: legal-services
description: "Use this skill for any task related to running, starting, or growing a Legal Services (Law Firm) business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: L
---

# ⚖️ Legal Services (Law Firm)

> Use this skill for any task related to running, starting, or growing a **Legal Services (Law Firm)** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Legal advisory, drafting, and litigation.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Professional Services |
| **Revenue Model** | Hourly fees, retainer, success fees |
| **Key Costs** | Staff, office, research tools |
| **Typical Team Size** | 2–20 |
| **Licenses Needed** | Bar Council enrolment |
| **Primary Tools** | Clio, MS Word, Zoom, DocuSign |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up Clio as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
