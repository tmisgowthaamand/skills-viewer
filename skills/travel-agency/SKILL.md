---
name: travel-agency
description: "Use this skill for any task related to running, starting, or growing a Travel Agency business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: T
---

# ✈️ Travel Agency

> Use this skill for any task related to running, starting, or growing a **Travel Agency** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Holiday packages, corporate travel, and ticketing.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Travel & Tourism |
| **Revenue Model** | Commissions, package margins |
| **Key Costs** | Staff, portal subscriptions, office |
| **Typical Team Size** | 3–20 |
| **Licenses Needed** | IATA accreditation (optional), GST |
| **Primary Tools** | Amadeus/Galileo, MakeMyTrip for agents, WhatsApp |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up Amadeus/Galileo as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
