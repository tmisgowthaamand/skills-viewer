---
name: transport-company
description: "Use this skill for any task related to running, starting, or growing a Transport Company business. Covers business planning, finance, marketing, operations, and compliance."
version: 1.0.0
category: business
letter: T
---

# 🚌 Transport Company

> Use this skill for any task related to running, starting, or growing a **Transport Company** business.
> Claude reads this file before executing finance, marketing, operations, or planning tasks for this business type.

## Skill Files in This Folder

| File | When to Use |
|------|-------------|
| [`business-plan.md`](./business-plan.md) | Writing a business plan, pitch deck, or strategy doc |
| [`finance.md`](./finance.md) | Budgets, P&L, invoicing, cash flow, tax, accounting |
| [`marketing.md`](./marketing.md) | Marketing plan, social media, SEO, ads, branding |
| [`operations.md`](./operations.md) | SOPs, hiring, staffing, tools, compliance, workflows |

## Business Overview

Passenger and goods transport fleet.

## Industry at a Glance

| Item | Details |
|------|---------|
| **Type** | Logistics/Transport |
| **Revenue Model** | Per-trip fees, contracts |
| **Key Costs** | Fleet, fuel, drivers, maintenance |
| **Typical Team Size** | 5–100 |
| **Licenses Needed** | Vehicle permits, transport contract licence, GST |
| **Primary Tools** | GPS tracking, Tally, Fleet management software |

## Quick Start Checklist

- [ ] Register business (GST + local licences)
- [ ] Set up GPS tracking as primary tool
- [ ] Create service/product menu with pricing
- [ ] Open Google My Business
- [ ] Set up WhatsApp Business
- [ ] Define 3 core SOPs

## Related Skills
- See [INDEX.md](../README.md) for related skills
